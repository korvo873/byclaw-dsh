...
/** Structural host services needed to assemble an activity snapshot. */
...
export async function assembleTeamSnapshot(
  ...
  if (options.historic !== true) {
    try {
      // These services may arrive through a linked plugin with another physical
      // copy of the branded SessionId type, so this module narrows them once.
      const subagents = ctx.subagents as {
        listChildren(parentSessionId: SessionId): Promise<ReadonlyArray<{ id: SessionId; kind: string }>>
      }
      const agents = ctx.agents as {
        get(sessionId: SessionId): { status: 'running' | 'idle' | 'ready' } | undefined
      }
      const children = await subagents.listChildren(state.captainSessionId as SessionId)
