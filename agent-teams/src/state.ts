...
/** Validate the complete controlled task graph before any member can run it. */
export function validateControlledTaskGraph(team: TeamState): string[] {
  const errors: string[] = []
  const activeMembers = new Set(team.members.filter(member => member.status !== 'removed').map(member => member.name))
  const byId = new Map(team.tasks.map(task => [task.id, task]))
  if (team.tasks.length === 0) errors.push('at least one task is required')
  for (const task of team.tasks) {
    if (task.description === undefined || task.description.trim() === '') errors.push(`${task.id} needs a non-empty description`)
    if (task.acceptanceCriteria === undefined || task.acceptanceCriteria.trim() === '') errors.push(`${task.id} needs non-empty acceptance criteria`)
    if (task.requiredTools === undefined) errors.push(`${task.id} must declare required_tools explicitly`)
    if (task.assignee === undefined) errors.push(`${task.id} must have an explicit assignee`)
    else if (task.assignee !== CAPTAIN_KEY && !activeMembers.has(task.assignee)) {
      errors.push(`${task.id} is assigned to missing member "${task.assignee}"`)
    }
    for (const dependency of task.dependencies) {
      if (!byId.has(dependency)) errors.push(`${task.id} depends on missing task ${dependency}`)
...
function isTeamTask(value: unknown): value is TeamTask {
  ...
  return typeof value['id'] === 'string'
    ...
    && isOptionalString(value['requesterSessionId'])
    && Array.isArray(value['dependencies'])
    && value['dependencies'].every((dependency) => typeof dependency === 'string')
    && isOptionalString(value['acceptanceCriteria'])
    && (value['requiredTools'] === undefined
      || (Array.isArray(value['requiredTools']) && value['requiredTools'].every(item => typeof item === 'string')))
    && (value['receipts'] === undefined
      || (Array.isArray(value['receipts']) && value['receipts'].every(isToolReceipt)))
    && (value['lastFailure'] === undefined || isTaskFailure(value['lastFailure']))
...
function isTeamState(value: unknown, expectedId: string): value is TeamState {
  ...
  const validShape = value['id'] === expectedId
    ...
    && isFiniteNumber(value['createdAt'])
    && (value['controlledWorkflow'] === undefined || typeof value['controlledWorkflow'] === 'boolean')
    && (value['lifecycle'] === undefined || value['lifecycle'] === 'draft' || value['lifecycle'] === 'running')
