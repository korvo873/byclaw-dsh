...
- 团队文件与归档快照是运行、UI 和恢复的真相源。`agent-teams/*` 会话事件只是尽力追加的信息记录；
  当前 Harness 不识别这些类型时会跳过，且追加失败绝不能使已成功的团队修改失效。
...
- `SessionEventMap` 可通过 `declare module '@deepseek-ai/dsh-session/types'` 扩展，但这不会把类型加入
  Harness 当前生成的已识别事件集合。未识别的 `agent-teams/*` 类型会被 emitter 跳过，不能作为 UI 或恢复数据源。
- AgentTeams 在团队文件修改成功后调用 [`appendTeamEvent`](../src/events.ts)，不把裸 `session.append` 作为工具契约。该 helper
  先检查运行时的已识别事件集合：不支持时直接返回；支持时仅用 `try/catch` 包住 `session.append`，追加失败只记录警告，已成功的团队文件修改保持有效。
...
- 若未来运行时注册了这些类型，emitter 会优先写入队长会话，队长不可达时回退调用者会话。无论类型未识别还是
  `session.append` 抛错，工具已成功写入的团队文件与归档快照都保持有效。
