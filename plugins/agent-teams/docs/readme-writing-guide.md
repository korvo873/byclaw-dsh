...
- 表格后补**数据流一句话**："工具执行 → 磁盘状态（真相源）→ host 快照路由 → 浮层轮询渲染。" 会话事件若由运行时识别才尽力追加，不能承诺作为重放或 UI 数据源。（一个方向链，不要画 ASCII 大图。）
...
- **0 层记录清单模板**（照此粒度记录）：
  ...
  - 落盘验证：以团队文件和归档快照作为运行、UI 和恢复的权威记录，核对提交回执和快照路由；会话事件只作尽力补充诊断，类型未识别而跳过或 `session.append` 失败都绝不能使已成功的团队修改失效
...
- 每条限制 = **现象 + 原因/影响 + 缓解**，一条 bullet 内说完。例：
  ...
  - "成员（模型）不总是严格走工具'仪式'（如完成时不调 `update_task`）——面板如实反映团队文件与 host 快照；队长以 `agent_teams_status`/文件为准汇总。"
