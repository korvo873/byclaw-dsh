...
- 判定成功：任务输出包含完整流程叙述（建队/成员/任务/产出/删队），且团队文件或归档快照包含相同的成员、任务和结果（见 2.3）
...
# 会话事件（可选诊断；只有 Harness 已识别类型时才会追加）
zstdcat ~/.dsh/sessions/<ws>/session-<id>/session.jsonl.zstd \
  | grep -o '"type":"agent-teams/[^"]*"' | sort | uniq -c
# 当前 Harness 未注册这些类型时，预期没有 agent-teams/* 记录。
# 若运行时已注册支持，可把记录用于补充诊断；不要将数量与团队修改一一对应。
```

验证以团队文件和归档快照为准：检查成员、任务、收件箱和结果是否与预期一致，并检查 host 快照路由返回同一状态。会话事件只是可选信息；未识别类型会跳过，`session.append` 失败也不能推翻已成功的团队修改。
...
- [ ] 团队状态文件、提交回执与活动快照路由反映同一团队；已识别的会话事件只能作为补充诊断
