...
> ByClaw 维护版本：包名为 `@byclaw/dsh-better-sidebar`，从本仓库的 `plugins/dsh-better-sidebar` 安装。运行时服务名、偏好命名空间和 DOM 标记保持兼容；上游版本见 [UPSTREAM.md](./UPSTREAM.md)。
...
- **🌿 Git 工作区**：源代码管理提供真 diff、历史、暂存 / 提交 / 还原与仓库选择器；独立的「工作树」tab 按仓库列出 linked worktree，可复制路径或通过主机校验的目标打开终端
...
当会话 CWD 根目录直接包含 `.git`（目录或文件）时，侧边栏首次初始化会确保「文件」与「源代码管理」两个 tab 存在；用户之后手动关闭的 tab 不会被自动补开。「源代码管理」顶部的仓库选择器列出根仓库和 `.gitmodules` 声明的递归子模块，未初始化或缺失的子模块仍会显示但不可执行 Git 操作。插件只读取 CWD、声明的子模块路径和 Git 自己的 worktree 清单，不扫描磁盘，也不会自动初始化或联网更新子模块。

「已暂存」与「未暂存」区域默认折叠，展开后按目录树显示变更；文件名和状态徽标同时用语义颜色区分新增、修改、删除/冲突、重命名/复制与未跟踪状态。源代码管理始终操作所选仓库的当前工作区，不提供 linked worktree 切换；位于会话 CWD 外部的 linked worktree 也不会获得文件编辑入口。

独立的「工作树」tab 从 `+` 菜单按需打开，按仓库分组显示分支、绝对路径、当前工作区、锁定状态和变更数量。第一版只提供刷新、复制路径和打开终端三种查看与定位操作，不提供创建、删除、锁定、解锁或修剪；打开终端时浏览器只传递不透明的仓库/worktree 标识，宿主重新校验清单后才决定终端 CWD，不接受客户端提供任意绝对路径。
