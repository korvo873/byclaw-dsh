...
| `git` | 20 | 是 | 否 | Git 面板 |
| `worktrees` | 25 | 是 | 否 | 只读工作树导航 |
| `subagent` | 30 | 是 | 否 | 子代理拓扑 |
| `sidechat` | 35 | 否（createTab 铸造 sidechat:`<uuid>`） | 否 | 侧边对话 |
...
- **`src/client/builtins/`**：8 个内置 tab（tabs.tsx）+ 6 个内置 viewer（viewers.tsx）的注册代码 + 聚合与 disposer 生命周期（index.ts）
