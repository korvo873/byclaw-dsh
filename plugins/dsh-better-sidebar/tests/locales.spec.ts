...
describe('locales (DSH i18n following)', () => {
  ...
  it('localizes semantic Git status labels used by accessible change rows', () => {
    const locale = new FakeLocale()
    attachLocale(locale)
    locale.switchTo('zh')
    expect(t('gitStatusModified')).toBe('已修改')
    expect(t('gitStatusConflicted')).toBe('冲突')
    locale.switchTo('en')
    expect(t('gitStatusModified')).toBe('Modified')
    expect(t('gitStatusConflicted')).toBe('Conflicted')
  })

  it('localizes the independent Worktrees tab in Chinese and English', () => {
    const locale = new FakeLocale()
    attachLocale(locale)
    locale.switchTo('zh')
    expect(t('worktrees')).toBe('工作树')
    locale.switchTo('en')
    expect(t('worktrees')).toBe('Worktrees')
  })
