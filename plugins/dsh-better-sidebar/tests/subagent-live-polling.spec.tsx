...
/** Expand the initially collapsed topology root. */
async function expandRoot(container: HTMLDivElement): Promise<void> {
  const button = container.querySelector<HTMLButtonElement>('button[aria-label^="展开 "]')
...
describe('SubagentView live polling', () => {
  it('sends one subagents.live per poll and never calls per-child history', async () => {
    ...
    await expandRoot(container)
    // Expanding performs the initial load; the first scheduled tick follows.
