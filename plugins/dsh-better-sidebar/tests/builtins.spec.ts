...
function setup(options: BuiltinTabOptions = {}): { ctx: Context; service: ReturnType<typeof createBetterSidebarService>; store: ReturnType<typeof createSidebarStore>; dispose: () => void } {
  const store = createSidebarStore()
  const service = createBetterSidebarService(store)
  const ctx = { betterSidebar: service } as Context
  const dispose = registerBuiltins(ctx, service, options)
  return { ctx, service, store, dispose }
}
...
describe('built-in tab registrations', () => {
  it('registers the 8 built-in tabs', () => {
    ...
    expect(service.getTabs().map(t => t.id).sort()).toEqual(
      ['browser', 'diff', 'editor', 'git', 'sidechat', 'subagent', 'terminal', 'worktrees'],
    )
  })
  ...
  it('single-instance tabs use the single sugar', () => {
    const { service } = setup()
    for (const id of ['git', 'subagent', 'worktrees']) {
      expect(service.getTab(id)?.single).toBe(true)
    }
  })

  it('registers localized Worktrees after Source Control in the + menu', () => {
    const { service } = setup()
    const descriptor = service.getTab('worktrees')
    expect(descriptor?.hidden).not.toBe(true)
    expect(descriptor?.order).toBeGreaterThan(service.getTab('git')!.order!)
    expect(descriptor?.order).toBeLessThan(service.getTab('subagent')!.order!)
    expect(typeof descriptor?.title === 'function' ? descriptor.title() : descriptor?.title).toBe(t('worktrees'))
  })

  it('opens a worktree terminal with only persisted opaque target metadata', () => {
    const { ctx, service, store } = setup({ terminalTitle: () => 'bash' })
    store.setSession('s1')
    const worktrees = service.getTab('worktrees')!.component({
      ctx,
      store,
      scope: { sessionId: 's1', cwd: '/workspace' },
      tab: { id: 'worktrees', type: 'worktrees', title: 'Worktrees' },
      visible: true,
    }) as ReactElement<{ onOpenTerminal: (target: { repositoryId: string; worktreeId: string }) => void }>
    worktrees.props.onOpenTerminal({ repositoryId: 'repository-root', worktreeId: 'worktree-linked' })
    const terminal = allLeaves(store.getSnapshot().state!.splits)
      .flatMap(leaf => leaf.tabs)
      .find(tab => tab.type === 'terminal')
    expect(terminal).toMatchObject({
      type: 'terminal',
      title: 'bash',
      meta: {
        terminalGitTarget: { repositoryId: 'repository-root', worktreeId: 'worktree-linked' },
      },
    })
    expect(terminal).not.toHaveProperty('path')
  })

  it('the Git descriptor restores and patches only its own opaque target metadata', () => {
    const { service, store } = setup()
    store.setSession('s1')
    service.openTab({ type: 'git' })
    const original = allLeaves(store.getSnapshot().state!.splits)
      .flatMap(leaf => leaf.tabs)
      .find(tab => tab.type === 'git')!
    const initialTarget = { repositoryId: 'repository-root', worktreeId: 'worktree-root' }
    const tab = { ...original, gitTarget: initialTarget }
    const descriptor = service.getTab('git')!
    const element = descriptor.component({
      ...
      initialTarget?: typeof initialTarget
      onTargetChange?: (target: typeof initialTarget) => void
    }>

    expect(element.props.initialTarget).toEqual(initialTarget)
    element.props.onTargetChange?.({ repositoryId: 'repository-child', worktreeId: 'worktree-child' })

    const restored = allLeaves(store.getSnapshot().state!.splits)
      .flatMap(leaf => leaf.tabs)
      .find(candidate => candidate.id === original.id)
    expect(restored?.gitTarget).toEqual({ repositoryId: 'repository-child', worktreeId: 'worktree-child' })
  })
