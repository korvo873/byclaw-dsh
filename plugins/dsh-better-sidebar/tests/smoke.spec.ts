...
describe('git destructive operations (scratch repository)', () => {
  ...
  it('reports a failing destructive operation as a GitCommandError', async () => {
    ...
    try {
      ...
      await expect(git.revert(dir, 'deadbeef00000000000000000000000000000000')).rejects.toBeInstanceOf(git.GitCommandError)
      await expect(git.cherryPick(dir, 'deadbeef00000000000000000000000000000000')).rejects.toBeInstanceOf(git.GitCommandError)
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
...
describe('session cwd resolution over the API route', () => {
  ...
  const gitTargetRun = (cwd: string, args: readonly string[]): string => {
    const result = spawnSync('git', ['-C', cwd, ...args], { encoding: 'utf8' })
    if (result.status !== 0) throw new Error(result.stderr || `git ${args.join(' ')} failed`)
    return result.stdout
  }

  const initializeTargetRepository = (path: string, content: string): void => {
    mkdirSync(path, { recursive: true })
    gitTargetRun(path, ['init', '-q', '-b', 'main'])
    gitTargetRun(path, ['config', 'user.name', 'dsh-better-sidebar-test'])
    gitTargetRun(path, ['config', 'user.email', 'test@dsh.invalid'])
    writeFileSync(join(path, 'shared.txt'), content)
    gitTargetRun(path, ['add', 'shared.txt'])
    gitTargetRun(path, ['commit', '-q', '-m', 'base'])
  }

  const targetFixture = (): { directory: string; root: string; child: string; childLinked: string } => {
    const directory = realpathSync(mkdtempSync(join(tmpdir(), 'dsh-sidebar-git-target-')))
    const root = join(directory, 'root')
    const child = join(root, 'packages', 'child')
    const childLinked = join(directory, 'child-linked')
    initializeTargetRepository(root, 'root base\n')
    initializeTargetRepository(child, 'child base\n')
    gitTargetRun(child, ['worktree', 'add', '-q', '-b', 'linked', childLinked])
    writeFileSync(join(root, '.gitmodules'), '[submodule "child"]\n\tpath = packages/child\n')
    return { directory, root, child, childLinked }
  }

  it('git target routes execute every operation against the selected repository worktree', async () => {
    const fixture = targetFixture()
    try {
      writeFileSync(join(fixture.root, 'shared.txt'), 'root changed\n')
      writeFileSync(join(fixture.child, 'shared.txt'), 'child primary changed\n')
      writeFileSync(join(fixture.childLinked, 'shared.txt'), 'child linked changed\n')
      const route = mount({ sessions: { get: () => ({ header: { cwd: fixture.root } }) } })
      const inventoryResponse = await invoke(route, 'git.inventory', { sessionId: 'git-target', refresh: true })
      expect(inventoryResponse.ok).toBe(true)
      const inventory = (inventoryResponse as unknown as {
        value: { repositories: Array<{ id: string; path: string; worktrees: Array<{ id: string; path: string; current: boolean }> }> }
      }).value
      const rootRepository = inventory.repositories.find(repository => repository.path === fixture.root)!
      const childRepository = inventory.repositories.find(repository => repository.path === fixture.child)!
      const childWorktree = childRepository.worktrees.find(worktree => !worktree.current)!
      const target = { repositoryId: childRepository.id, worktreeId: childWorktree.id }

      const status = await invoke(route, 'git.status', { sessionId: 'git-target', target })
      expect(status).toMatchObject({ ok: true, value: { root: fixture.childLinked } })
      const diff = await invoke(route, 'git.diff', { sessionId: 'git-target', target, path: 'shared.txt', staged: false })
      expect((diff as unknown as { value: { diff: string } }).value.diff).toContain('+child linked changed')

      expect((await invoke(route, 'git.stage', { sessionId: 'git-target', target, path: 'shared.txt' })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe('shared.txt')
      expect(gitTargetRun(fixture.child, ['diff', '--cached', '--name-only']).trim()).toBe('')
      expect(gitTargetRun(fixture.root, ['diff', '--cached', '--name-only']).trim()).toBe('')
      expect((await invoke(route, 'git.unstage', { sessionId: 'git-target', target, path: 'shared.txt' })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe('')

      await invoke(route, 'git.stage', { sessionId: 'git-target', target, path: 'shared.txt' })
      expect((await invoke(route, 'git.commit', { sessionId: 'git-target', target, message: 'child route commit' })).ok).toBe(true)
      const committedHash = gitTargetRun(fixture.childLinked, ['rev-parse', 'HEAD']).trim()
      gitTargetRun(fixture.childLinked, ['branch', 'feature'])
      const branches = await invoke(route, 'git.branch', { sessionId: 'git-target', target })
      expect(branches).toMatchObject({
        ok: true,
        value: {
          current: 'linked',
          names: expect.arrayContaining(['linked', 'feature']),
          local: expect.arrayContaining(['linked', 'feature']),
          remote: [],
        },
      })
      expect((await invoke(route, 'git.checkout', { sessionId: 'git-target', target, branch: 'feature' })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['branch', '--show-current']).trim()).toBe('feature')

      const log = await invoke(route, 'git.log', {
        sessionId: 'git-target',
        target,
        query: { scope: 'current', count: 1, skip: 0 },
      })
      expect(log).toMatchObject({
        ok: true,
        value: {
          entries: [{ hashFull: committedHash, subject: 'child route commit', parents: expect.any(Array) }],
          hasMore: true,
        },
      })
      const commitDiff = await invoke(route, 'git.commit-diff', { sessionId: 'git-target', target, hash: committedHash })
      expect((commitDiff as unknown as { value: { diff: string } }).value.diff).toContain('+child linked changed')
      const shown = await invoke(route, 'git.show', { sessionId: 'git-target', target, rev: 'HEAD', path: 'shared.txt' })
      expect(shown).toMatchObject({ ok: true, value: { content: 'child linked changed\n' } })

      writeFileSync(join(fixture.childLinked, 'shared.txt'), 'discard me\n')
      expect((await invoke(route, 'git.discard', { sessionId: 'git-target', target, path: 'shared.txt' })).ok).toBe(true)
      expect(readFileSync(join(fixture.childLinked, 'shared.txt'), 'utf8')).toBe('child linked changed\n')
      expect(readFileSync(join(fixture.child, 'shared.txt'), 'utf8')).toBe('child primary changed\n')
      expect(readFileSync(join(fixture.root, 'shared.txt'), 'utf8')).toBe('root changed\n')

      expect((await invoke(route, 'git.revert', { sessionId: 'git-target', target, hash: committedHash })).ok).toBe(true)
      expect(readFileSync(join(fixture.childLinked, 'shared.txt'), 'utf8')).toBe('child base\n')
      gitTargetRun(fixture.childLinked, ['checkout', '-q', '-b', 'pick-source'])
      writeFileSync(join(fixture.childLinked, 'picked.txt'), 'picked\n')
      gitTargetRun(fixture.childLinked, ['add', 'picked.txt'])
      gitTargetRun(fixture.childLinked, ['commit', '-q', '-m', 'pick source'])
      const pickedHash = gitTargetRun(fixture.childLinked, ['rev-parse', 'HEAD']).trim()
      gitTargetRun(fixture.childLinked, ['checkout', '-q', 'feature'])
      expect((await invoke(route, 'git.cherry-pick', { sessionId: 'git-target', target, hash: pickedHash })).ok).toBe(true)
      expect(readFileSync(join(fixture.childLinked, 'picked.txt'), 'utf8')).toBe('picked\n')
      expect(rootRepository.worktrees.some(worktree => worktree.id === childWorktree.id)).toBe(false)
    } finally {
      rmSync(fixture.directory, { recursive: true, force: true })
    }
  })

  it('git target validation rejects unknown pairs and unsafe relative paths', async () => {
    const fixture = targetFixture()
    try {
      const route = mount({ sessions: { get: () => ({ header: { cwd: fixture.root } }) } })
      const inventoryResponse = await invoke(route, 'git.inventory', { sessionId: 'git-target-validation', refresh: true })
      const repositories = (inventoryResponse as unknown as {
        value: { repositories: Array<{ id: string; worktrees: Array<{ id: string }> }> }
      }).value.repositories
      const root = repositories[0]!
      const child = repositories[1]!
      const validTarget = { repositoryId: child.id, worktreeId: child.worktrees[0]!.id }
      const invalidTargets = [
        { repositoryId: 'unknown-repository', worktreeId: child.worktrees[0]!.id },
        { repositoryId: child.id, worktreeId: root.worktrees[0]!.id },
      ]
      for (const target of invalidTargets) {
        const result = await invoke(route, 'git.status', { sessionId: 'git-target-validation', target })
        expect(result).toMatchObject({ ok: false, status: 400, error: { code: 'bad-request' } })
      }
      ...
      for (const [method, extra] of pathRoutes) {
        const result = await invoke(route, method, {
          sessionId: 'git-target-validation',
          target: validTarget,
          path: '../outside.txt',
          ...extra,
        })
      ...
      for (const path of ['', '/tmp/absolute.txt', 'nested/../../outside.txt', 'nul\0path']) {
        const result = await invoke(route, 'git.diff', {
          sessionId: 'git-target-validation', target: validTarget, path, staged: false,
        })
        expect(result).toMatchObject({ ok: false, status: 400, error: { code: 'bad-request' } })
      }
    } finally {
  ...
  it('git batch paths validate every exact path before staging or unstaging the selected target', async () => {
    const fixture = targetFixture()
    try {
      const nested = join(fixture.childLinked, 'nested')
      mkdirSync(nested, { recursive: true })
      writeFileSync(join(nested, 'one.txt'), 'one\n')
      writeFileSync(join(nested, 'two.txt'), 'two\n')
      writeFileSync(join(fixture.childLinked, 'outside.txt'), 'outside\n')
      writeFileSync(join(fixture.child, 'outside.txt'), 'primary outside\n')
      writeFileSync(join(fixture.root, 'outside.txt'), 'root outside\n')

      const route = mount({ sessions: { get: () => ({ header: { cwd: fixture.root } }) } })
      const inventoryResponse = await invoke(route, 'git.inventory', { sessionId: 'git-batch', refresh: true })
      const repositories = (inventoryResponse as unknown as {
        value: { repositories: Array<{ id: string; path: string; worktrees: Array<{ id: string; current: boolean }> }> }
      }).value.repositories
      const child = repositories.find(repository => repository.path === fixture.child)!
      const linked = child.worktrees.find(worktree => !worktree.current)!
      const target = { repositoryId: child.id, worktreeId: linked.id }

      for (const paths of [
        undefined,
        [],
        'nested/one.txt',
        ['nested/one.txt', '../outside.txt'],
        ['nested/one.txt', '.'],
        ['nested/one.txt', 'nested/'],
      ]) {
        const result = await invoke(route, 'git.stage-paths', { sessionId: 'git-batch', target, ...(paths === undefined ? {} : { paths }) })
        expect(result).toMatchObject({ ok: false, status: 400, error: { code: 'bad-request' } })
      }
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe('')

      expect((await invoke(route, 'git.stage-paths', {
        sessionId: 'git-batch', target, paths: ['nested/one.txt', 'nested/two.txt'],
      })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim().split('\n')).toEqual([
        'nested/one.txt',
        'nested/two.txt',
      ])
      expect(gitTargetRun(fixture.childLinked, ['status', '--short', 'outside.txt']).trim()).toBe('?? outside.txt')
      expect(gitTargetRun(fixture.child, ['diff', '--cached', '--name-only']).trim()).toBe('')
      expect(gitTargetRun(fixture.root, ['diff', '--cached', '--name-only']).trim()).toBe('')

      gitTargetRun(fixture.childLinked, ['add', 'outside.txt'])
      expect((await invoke(route, 'git.unstage-paths', {
        sessionId: 'git-batch', target, paths: ['nested/one.txt', 'nested/two.txt'],
      })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe('outside.txt')

      const invalidUnstage = await invoke(route, 'git.unstage-paths', {
        sessionId: 'git-batch', target, paths: ['outside.txt', '/tmp/absolute.txt'],
      })
      expect(invalidUnstage).toMatchObject({ ok: false, status: 400, error: { code: 'bad-request' } })
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe('outside.txt')
    } finally {
      rmSync(fixture.directory, { recursive: true, force: true })
    }
  ...
  it('git batch paths stage a magic-looking filename literally', async () => {
    const fixture = targetFixture()
    try {
      for (const path of [':(glob)*.txt', 'alpha.txt', 'beta.txt']) {
        writeFileSync(join(fixture.childLinked, path), `${path}\n`)
      }
      const route = mount({ sessions: { get: () => ({ header: { cwd: fixture.root } }) } })
      const inventoryResponse = await invoke(route, 'git.inventory', { sessionId: 'git-batch-literal-stage', refresh: true })
      const repositories = (inventoryResponse as unknown as {
        value: { repositories: Array<{ id: string; path: string; worktrees: Array<{ id: string; current: boolean }> }> }
      }).value.repositories
      const child = repositories.find(repository => repository.path === fixture.child)!
      ...
      expect((await invoke(route, 'git.stage-paths', {
        sessionId: 'git-batch-literal-stage', target, paths: [':(glob)*.txt'],
      })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim()).toBe(':(glob)*.txt')
      expect(gitTargetRun(fixture.childLinked, ['status', '--short', 'alpha.txt', 'beta.txt']).trim().split('\n')).toEqual([
        '?? alpha.txt',
        '?? beta.txt',
      ])
    } finally {
  ...
  it('git batch paths unstage a magic-looking filename literally', async () => {
    const fixture = targetFixture()
    try {
      for (const path of [':(glob)*.txt', 'alpha.txt', 'beta.txt']) {
        writeFileSync(join(fixture.childLinked, path), `${path}\n`)
      }
      gitTargetRun(fixture.childLinked, ['add', '-A'])
      const route = mount({ sessions: { get: () => ({ header: { cwd: fixture.root } }) } })
      const inventoryResponse = await invoke(route, 'git.inventory', { sessionId: 'git-batch-literal-unstage', refresh: true })
      const repositories = (inventoryResponse as unknown as {
        value: { repositories: Array<{ id: string; path: string; worktrees: Array<{ id: string; current: boolean }> }> }
      }).value.repositories
      const child = repositories.find(repository => repository.path === fixture.child)!
      ...
      expect((await invoke(route, 'git.unstage-paths', {
        sessionId: 'git-batch-literal-unstage', target, paths: [':(glob)*.txt'],
      })).ok).toBe(true)
      expect(gitTargetRun(fixture.childLinked, ['diff', '--cached', '--name-only']).trim().split('\n')).toEqual([
        'alpha.txt',
        'beta.txt',
      ])
      expect(gitTargetRun(fixture.childLinked, ['status', '--short']).trim().split('\n')).toContain('?? :(glob)*.txt')
    } finally {
      rmSync(fixture.directory, { recursive: true, force: true })
    }
  })
  ...
  it('git target routes reject path-only diff requests', async () => {
    const route = mount({
      sessions: {
