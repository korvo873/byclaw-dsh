...
describe('tab meta (v0.12.0)', () => {
  ...
  it('applies seed meta to a tab minted by createTab under the existing tabMeta capability', () => {
    const store = createSidebarStore()
    const service = createBetterSidebarService(store)
    service.registerTab({
      id: 'minted',
      title: 'Minted',
      createTab: () => ({ tab: { id: 'minted:1', type: 'minted', title: 'Minted' } }),
      component: () => null,
    })
    store.setSession('s1')
    service.openTab({ type: 'minted', title: 'Caller title', meta: { target: 'opaque' } })
    const tab = allLeaves(store.getSnapshot().state!.splits).flatMap(leaf => leaf.tabs)
      .find(candidate => candidate.id === 'minted:1')
    expect(tab?.meta).toEqual({ target: 'opaque' })
    expect(tab?.title).toBe('Minted')
  })
