...
function packedManifest(): Record<string, unknown> {
  ...
  try {
    ...
    const archive = readdirSync(dir).find(name => name.endsWith('.tgz'))
    if (archive === undefined) throw new Error('pnpm pack did not produce a tarball')
    execFileSync('tar', ['-xzf', join(dir, archive), '-C', dir, 'package/package.json'], { stdio: 'pipe' })
