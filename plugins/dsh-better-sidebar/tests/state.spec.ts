...
import {
  ...
  initializeGitDefaultTabs, migrateBottomTabs, moveFloat, moveTab, moveTabToEdge, openDiffTab,
...
describe('sidebar state', () => {
  ...
  describe('Git default tabs', () => {
    const initialize = (state: SidebarState, overrides: Partial<Parameters<typeof initializeGitDefaultTabs>[1]> = {}) =>
      initializeGitDefaultTabs(state, {
        cwdHasGitEntry: true,
        editorEnabled: true,
        gitEnabled: true,
        ...overrides,
    ...
    it('uses the active locale for fresh default tab titles', () => {
      attachLocale({ getSnapshot: () => ({ active: 'zh' }) })
      try {
        const initialized = initialize(makeDefaultState(400, true, 'none'))
        const leaf = allLeaves(initialized.splits)[0]!

        expect(leaf.tabs.map(tab => tab.title)).toEqual(['文件', '源代码管理'])
      } finally {
        attachLocale(undefined)
      }
    })

    it('seeds Files then Source Control once and keeps Files active', () => {
      const initialized = initialize(makeDefaultState())
      const leaf = allLeaves(initialized.splits)[0]!

      expect(leaf.tabs.map(tab => tab.type)).toEqual(['editor', 'git'])
      expect(leaf.tabs.map(tab => tab.id)).toEqual([expect.any(String), 'git'])
    ...
    it('uses stable ids when adding both defaults to an empty state', () => {
      const initialized = initialize(makeDefaultState(400, true, 'none'))
      const leaf = allLeaves(initialized.splits)[0]!

      expect(leaf.tabs).toEqual([
        { id: 'editor', type: 'editor', title: 'Files', meta: { treeOpen: true } },
        { id: 'git', type: 'git', title: 'Source Control' },
      ])
      expect(leaf.active).toBe('editor')
    })

    it('adds a distinct Files home when only a path editor exists', () => {
      const original = makeDefaultState(400, true, 'none')
      const leaf = allLeaves(original.splits)[0]!
      const initialized = initialize({
        ...original,
        splits: {
          ...leaf,
          tabs: [{ id: 'editor', type: 'editor', title: 'a.ts', path: '/workspace/a.ts' }],
          active: 'editor',
        },
      })
      const tabs = allLeaves(initialized.splits)[0]!.tabs

      expect(tabs.map(tab => tab.id)).toEqual(['editor', 'editor:home', 'git'])
      expect(tabs.find(tab => tab.id === 'editor:home')).toEqual({
        id: 'editor:home', type: 'editor', title: 'Files', meta: { treeOpen: true },
      })
      expect(allLeaves(initialized.splits)[0]!.active).toBe('editor:home')
      expect(tabs.some(tab => tab.type === 'worktrees')).toBe(false)
    })

    it('probes a unique Files id when path editors occupy both stable candidates', () => {
      const original = makeDefaultState(400, true, 'none')
      const leaf = allLeaves(original.splits)[0]!
      const initialized = initialize({
        ...original,
        splits: {
          ...leaf,
          tabs: [
            { id: 'editor', type: 'editor', title: 'a.ts', path: '/workspace/a.ts' },
            { id: 'editor:home', type: 'editor', title: 'b.ts', path: '/workspace/b.ts' },
          ],
          active: 'editor:home',
        },
      })
      const initializedLeaf = allLeaves(initialized.splits)[0]!
      const ids = initializedLeaf.tabs.map(tab => tab.id)
      const filesIndex = ids.indexOf('editor:home:2')

      expect(ids).toEqual(['editor', 'editor:home', 'editor:home:2', 'git'])
      expect(new Set(ids).size).toBe(ids.length)
      expect(initializedLeaf.tabs[filesIndex]).toEqual({
        id: 'editor:home:2', type: 'editor', title: 'Files', meta: { treeOpen: true },
      })
      expect(initializedLeaf.active).toBe('editor:home:2')
      expect(initializedLeaf.tabs[filesIndex + 1]?.type).toBe('git')
      expect(initializedLeaf.tabs.some(tab => tab.type === 'worktrees')).toBe(false)
    })

    it('adds Source Control after a Files home in its bottom pane', () => {
      const original = makeDefaultState(400, true, 'none')
      const right = allLeaves(original.splits)[0]!
      const bottom = allLeaves(original.bottomSplits)[0]!
      const initialized = initialize({
        ...original,
        activePane: right.id,
        splits: {
          ...right,
          tabs: [{ id: 'editor:file', type: 'editor', title: 'a.ts', path: '/workspace/a.ts' }],
          active: 'editor:file',
        },
        bottomSplits: {
          ...bottom,
          tabs: [{ id: 'files-bottom', type: 'editor', title: 'Files', meta: { treeOpen: true } }],
          active: 'files-bottom',
        },
      })
      const bottomAfter = allLeaves(initialized.bottomSplits)[0]!

      expect(allLeaves(initialized.splits)[0]!.tabs.map(tab => tab.id)).toEqual(['editor:file'])
      expect(bottomAfter.tabs.map(tab => tab.id)).toEqual(['files-bottom', 'git'])
      expect(bottomAfter.active).toBe('files-bottom')
      expect(initialized.activePane).toBe(bottomAfter.id)
      expect(bottomAfter.tabs.some(tab => tab.type === 'worktrees')).toBe(false)
    })

    it('does not add disabled default tab types', () => {
      const editorOnly = initialize(makeDefaultState(400, true, 'none'), { gitEnabled: false })
      const gitOnly = initialize(makeDefaultState(400, true, 'none'), { editorEnabled: false })
      const neither = initialize(makeDefaultState(400, true, 'none'), { editorEnabled: false, gitEnabled: false })

      expect(allLeaves(editorOnly.splits).flatMap(leaf => leaf.tabs).map(tab => tab.type)).toEqual(['editor'])
      expect(allLeaves(gitOnly.splits).flatMap(leaf => leaf.tabs).map(tab => tab.type)).toEqual(['git'])
      expect(allLeaves(neither.splits).flatMap(leaf => leaf.tabs)).toEqual([])
    })

    it('only records the check when the original CWD has no literal .git entry', () => {
      const original = makeDefaultState()
      const initialized = initialize(original, { cwdHasGitEntry: false })

      expect(initialized.gitDefaultsChecked).toBe(true)
      expect(initialized.splits).toBe(original.splits)
      expect(allLeaves(initialized.splits).flatMap(leaf => leaf.tabs).map(tab => tab.type)).toEqual(['editor'])
    })

    it('migrates an older persisted state with the check still pending', () => {
      const persisted = JSON.parse(JSON.stringify(makeDefaultState())) as Record<string, unknown>
      delete persisted.gitDefaultsChecked

      expect(sanitizeState(persisted)?.gitDefaultsChecked).toBe(false)
    })

    it('does not recreate Source Control after the user closes it', () => {
      const initialized = initialize(makeDefaultState())
      const leaf = allLeaves(initialized.splits)[0]!
      const closed = closeTab(initialized, leaf.id, 'git')

      expect(allLeaves(closed.splits).flatMap(candidate => candidate.tabs).map(tab => tab.type)).toEqual(['editor'])
      expect(initialize(closed)).toBe(closed)
    })
  })
...
describe('v0.12.0 store additions', () => {
  ...
  describe('tab meta persistence (v0.12.0)', () => {
    ...
    it('sanitizeState retains only opaque string ids from a Git tab target', () => {
      const store = createSidebarStore()
      store.setSession('s1')
      store.reduce(s => ({
        ...s,
      ...
      const sanitized = sanitizeState(JSON.parse(JSON.stringify(store.getSnapshot().state!)))!
      const tab = allLeaves(sanitized.splits).flatMap(leaf => leaf.tabs)[0]
      expect(tab?.gitTarget).toEqual({ repositoryId: 'repository-root', worktreeId: 'worktree-root' })
    })

    it('sanitizeState retains only opaque string ids from a worktree terminal target', () => {
      ...
      const sanitized = sanitizeState({
        ...
        splits: {
          ...
          active: 'terminal:worktree',
        },
      })!
      const terminal = allLeaves(sanitized.splits).flatMap(candidate => candidate.tabs)[0]
      expect(terminal?.meta).toEqual({
        terminalGitTarget: {
          repositoryId: 'repository-root',
          worktreeId: 'worktree-linked',
        },
