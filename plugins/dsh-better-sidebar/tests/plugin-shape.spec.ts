...
type SidebarPackageRoot = typeof import('../src/index.ts')
// @ts-expect-error Terminal route internals are not package-root API.
type ForbiddenAttachTerminal = SidebarPackageRoot['attachTerminal']
// @ts-expect-error Terminal cwd resolution is not package-root API.
type ForbiddenTerminalCwdResolver = SidebarPackageRoot['resolveTerminalWorkingDirectory']
void (undefined as unknown as ForbiddenAttachTerminal)
void (undefined as unknown as ForbiddenTerminalCwdResolver)
...
describe('dsh-better-sidebar plugin export shape', () => {
  it('has the namespace-plugin export shape (no stray default) so the Loader keeps name/inject/Config/apply', () => {
    ...
    expect('attachTerminal' in sidebar).toBe(false)
    expect('resolveTerminalWorkingDirectory' in sidebar).toBe(false)
