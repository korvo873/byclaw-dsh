...
> ByClaw-maintained package: install `@byclaw/dsh-better-sidebar` from this repository's `plugins/dsh-better-sidebar` directory. Runtime service names, preference namespaces, and DOM markers remain compatible. See [UPSTREAM.md](./UPSTREAM.md) for provenance.
...
- **🌿 Git Workspaces**: Source Control provides real diffs, history, stage / commit / revert, and a repository selector; the independent **Worktrees** tab groups linked worktrees by repository and can copy a path or open a terminal through a host-validated target
...
When the session CWD root directly contains a `.git` directory or file, the first sidebar initialization ensures that Files and Source Control tabs exist. Tabs that the user later closes are not reopened automatically. The repository selector at the top of Source Control lists the root repository and recursively declared `.gitmodules` children; uninitialized or missing submodules remain visible but cannot run Git operations. Discovery reads only the CWD, declared submodule paths, and Git's own worktree list. It never scans the disk or initializes or updates a submodule over the network.

Staged and Unstaged sections start collapsed and expand into directory trees. File names and status badges use semantic colors while retaining text labels for added, modified, deleted/conflicted, renamed/copied, and untracked states. Source Control always operates on the selected repository's current checkout and does not expose a linked-worktree selector. Linked worktrees outside the session CWD do not gain an editor entry point.

Open the independent Worktrees tab from the `+` menu to view worktrees grouped by repository, including branch, absolute path, current-checkout marker, lock state, and change count. The first version is read-only navigation: refresh, copy path, and open terminal only; it does not create, remove, lock, unlock, or prune worktrees. Opening a terminal sends opaque repository/worktree IDs, and the host revalidates the inventory before choosing the terminal CWD instead of trusting an arbitrary client-supplied absolute path.
