...
import type { GitTarget } from './api.ts'
import { terminalGitTargetOf } from './terminal-target.ts'
...
export type SidebarDiffRef =
  | { kind: 'worktree'; path: string; staged: boolean; untracked?: boolean; target: GitTarget }
  | { kind: 'commit'; hash: string; hashFull: string; subject: string; target: GitTarget }
...
export interface SidebarTab {
  ...
  path?: string
  diff?: SidebarDiffRef
  /** Source Control's persisted repository selection. Only opaque host-issued
   *  ids cross the persistence boundary; repository paths are never stored. */
  gitTarget?: GitTarget
...
export interface SidebarState {
  panelOpen: boolean
  width: number
  /** Whether this session has completed its one-time Git default-tab check. */
  gitDefaultsChecked: boolean
...
export function makeDefaultState(width = PANEL_DEFAULT, panelOpen = true, seed: DefaultSeed = 'editor-home'): SidebarState {
  ...
  if (seed === 'editor-home') {
    ...
    leaf.tabs = [{ id: uid('tab'), type: 'editor', title: t('files'), meta: { treeOpen: true } }]
...
/**
 * Complete the one-time default-tab check for a session. A literal `.git`
 * entry in the original CWD may seed Files and Source Control; nested Git
 * membership alone never does. Existing tabs win, and stable ids keep a
 * retried initialization from minting duplicates.
 * @param state - the current per-session sidebar state.
 * @param options - inventory result and current tab-type enablement.
 * @returns the initialized state, or the same reference after an earlier check.
 */
export function initializeGitDefaultTabs(
  state: SidebarState,
  options: { cwdHasGitEntry: boolean; editorEnabled: boolean; gitEnabled: boolean },
): SidebarState {
  if (state.gitDefaultsChecked) return state
  if (!options.cwdHasGitEntry) return { ...state, gitDefaultsChecked: true }

  const rightLeaves = allLeaves(state.splits)
  const bottomLeaves = allLeaves(state.bottomSplits)
  const openTabs = rightLeaves.concat(bottomLeaves).flatMap(leaf => leaf.tabs)
    .concat(state.floats.map(float => float.tab))
  const isFilesHome = (tab: SidebarTab): boolean => {
    if (tab.type !== 'editor' || tab.path !== undefined) return false
    if (tab.meta === null || typeof tab.meta !== 'object' || Array.isArray(tab.meta)) return true
    return (tab.meta as Record<string, unknown>).dir !== true
  }
  const rightFilesLeaf = rightLeaves.find(leaf => leaf.tabs.some(isFilesHome))
  const bottomFilesLeaf = bottomLeaves.find(leaf => leaf.tabs.some(isFilesHome))
  // Pane groups can place Source Control beside Files. A float owns exactly
  // one tab, so a floated Files home only suppresses a duplicate home; Git
  // falls back to the active pane below.
  const filesPane = rightFilesLeaf === undefined
    ? bottomFilesLeaf === undefined ? undefined : { key: 'bottomSplits' as const, leaf: bottomFilesLeaf }
    : { key: 'splits' as const, leaf: rightFilesLeaf }
  const filesHome = filesPane?.leaf.tabs.find(isFilesHome) ?? state.floats.map(float => float.tab).find(isFilesHome)
  const editorToAdd = options.editorEnabled && filesHome === undefined
  const gitToAdd = options.gitEnabled && !openTabs.some(tab => tab.type === 'git')

  const activeRight = rightLeaves.find(leaf => leaf.id === state.activePane)
  const activeBottom = bottomLeaves.find(leaf => leaf.id === state.activePane)
  const target = filesPane
    ?? (activeBottom === undefined
      ? { key: 'splits' as const, leaf: activeRight ?? firstLeaf(state.splits) }
      : { key: 'bottomSplits' as const, leaf: activeBottom })
  const filesId = filesPane?.leaf.tabs.find(isFilesHome)?.id
  const targetAlreadyActive = filesId === undefined
    || (target.leaf.active === filesId && state.activePane === target.leaf.id)
  if (!editorToAdd && !gitToAdd && targetAlreadyActive) return { ...state, gitDefaultsChecked: true }

  const usedTabIds = new Set(openTabs.map(tab => tab.id))
  let defaultEditorId = 'editor'
  if (usedTabIds.has(defaultEditorId)) {
    defaultEditorId = 'editor:home'
    let suffix = 2
    while (usedTabIds.has(defaultEditorId)) {
      defaultEditorId = `editor:home:${suffix}`
      suffix += 1
    }
  }
  const targetTree = mapLeaf(state[target.key], target.leaf.id, (leaf) => {
    let tabs = leaf.tabs
    let editorId = filesId
    if (editorToAdd) {
      const editor: SidebarTab = { id: defaultEditorId, type: 'editor', title: t('files'), meta: { treeOpen: true } }
      tabs = [...tabs, editor]
      editorId = editor.id
    }
    if (gitToAdd) {
      const git: SidebarTab = { id: 'git', type: 'git', title: t('git') }
      const editorIndex = editorId === undefined ? -1 : tabs.findIndex(tab => tab.id === editorId)
      tabs = editorIndex < 0
        ? [...tabs, git]
        : [...tabs.slice(0, editorIndex + 1), git, ...tabs.slice(editorIndex + 1)]
    }
    leaf.tabs = tabs
    if (editorId !== undefined && tabs.some(tab => tab.id === editorId)) leaf.active = editorId
    else if (leaf.active === null && gitToAdd) leaf.active = 'git'
  })
  return { ...state, gitDefaultsChecked: true, [target.key]: targetTree, activePane: target.leaf.id }
}
...
/** Update the persisted fields of one open tab (title / path / meta / Git target) without
...
export function patchTab(
  state: SidebarState,
  tabId: string,
  patch: { title?: string; path?: string; meta?: unknown; gitTarget?: GitTarget | null },
): SidebarState {
  let changed = false
  const apply = (tab: SidebarTab): SidebarTab => {
    ...
    return {
      ...
      ...(patch.gitTarget === undefined
        ? {}
        : patch.gitTarget === null ? { gitTarget: undefined } : { gitTarget: patch.gitTarget }),
    }
  }
...
export function sanitizeState(parsed: unknown): SidebarState | undefined {
  ...
  return {
    ...
    // Older persisted sessions have not run the default-tab check yet.
    gitDefaultsChecked: record.gitDefaultsChecked === true,
...
function sanitizePersistedTab(tab: unknown): SidebarTab | 'diff' | undefined {
  ...
  if (candidate.type === 'explorer') {
    const meta = candidate.meta !== null && typeof candidate.meta === 'object' && !Array.isArray(candidate.meta)
      ...
      : undefined
    return { id: candidate.id, type: 'editor', title: t('files'), meta: { treeOpen: true, ...meta } }
  }
  const rawGitTarget = candidate.gitTarget !== null && typeof candidate.gitTarget === 'object'
    ? candidate.gitTarget as Record<string, unknown>
    : undefined
  const gitTarget = typeof rawGitTarget?.repositoryId === 'string' && typeof rawGitTarget.worktreeId === 'string'
    ? { repositoryId: rawGitTarget.repositoryId, worktreeId: rawGitTarget.worktreeId }
    : undefined
  ...
  return {
    ...
    ...(typeof candidate.path === 'string'
      && !(candidate.type === 'terminal' && terminalGitTargetOf(candidate.meta) !== undefined)
      ? { path: candidate.path }
      : {}),
    ...(gitTarget === undefined ? {} : { gitTarget }),
