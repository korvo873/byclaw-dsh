...
export function createBetterSidebarService(store: SidebarStore): BetterSidebarService {
  ...
  const openTab = (seed: OpenTabSeed, scope?: SessionScope): void => {
    ...
    const reducer = (state: SidebarState): SidebarState => {
      ...
      // URL and metadata seeds apply to a NEWLY CREATED tab even when its
      // descriptor mints the id through createTab; a FOCUS must never be
      ...
      if (isCreation && (seed.url !== undefined || seed.meta !== undefined)) {
        landed = patchTab(next, tab.id, {
          ...(seed.url !== undefined ? { path: seed.url } : {}),
          ...(seed.url !== undefined && seed.title !== undefined ? { title: seed.title } : {}),
          ...(seed.meta !== undefined ? { meta: seed.meta } : {}),
        })
      }
