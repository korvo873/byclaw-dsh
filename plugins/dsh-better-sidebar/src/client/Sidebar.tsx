...
import {
  ...
  initializeGitDefaultTabs, moveFloat, moveTab, moveTabToEdge, openDiffTab, raiseFloat, reconcileAgentTerminals,
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  // Once the original session CWD is known, seed Files and Source Control
  // only when that exact directory has a literal .git entry. A rejected
  // inventory request leaves the persisted marker pending for a later
  // remount, while this effect's stable dependencies prevent a retry loop.
  useEffect(() => {
    if (sessionId === undefined || cwd === undefined || state === undefined || state.gitDefaultsChecked) return
    const controller = new AbortController()
    api.gitInventory({ sessionId, cwd }, false, controller.signal)
      .then((inventory) => {
        if (controller.signal.aborted) return
        const initialize = (current: SidebarState): SidebarState => initializeGitDefaultTabs(current, {
        ...
        if (store.getSnapshot().sessionId === sessionId) store.reduce(initialize)
        else store.reduceFor(sessionId, initialize)
      })
      .catch(() => { /* Leave the marker pending so a later mount can retry. */ })
    return () => { controller.abort() }
  }, [sessionId, cwd, state?.gitDefaultsChecked, store])
