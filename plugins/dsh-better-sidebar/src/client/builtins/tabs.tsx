 ...
 * (editor / git / worktrees / subagent / sidechat / terminal / browser / diff) through
...
import { allLeaves, isAgentTabId, patchTab, type SidebarState } from '../state.ts'
...
import { WorktreesView } from '../WorktreesView.tsx'
...
interface TerminalViewProps {
  ...
  tabId: string
  store: SidebarStore
  target?: import('../terminal-target.ts').TerminalGitTarget
}
  ...
  return [
    ...
    },
    {
      ...
      component: ({ ctx, store, scope, tab, visible, onOpenDiff }) => (
        <GitView
          scope={scope}
          visible={visible}
          initialTarget={tab.gitTarget}
          onTargetChange={(gitTarget) => {
            store.reduce(state => patchTab(state, tab.id, { gitTarget }))
          }}
    ...
    },
    {
      ...
      component: ({ ctx, scope }) => (
        <WorktreesView
          scope={scope}
          onOpenTerminal={(terminalGitTarget) => {
            ctx.betterSidebar.openTab({ type: 'terminal', meta: { terminalGitTarget } }, scope)
          }}
        />
      ),
    },
    ...
    },
    {
      ...
      component: ({ tab, scope, store }) => (
        <LazyTerminal scope={scope} store={store} tabId={tab.id} target={terminalGitTargetOf(tab.meta)} />
      ),
    },
    {
      id: 'browser',
