...
export function DiffTab(props: { sessionId: string; cwd: string | undefined; diff: SidebarDiffRef }) {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        if (diff.kind === 'commit') {
          const result = await api.gitCommitDiff(scope, diff.target, diff.hashFull)
          if (!cancelled) setData({ diff: result.diff })
          return
        }
        let result = await api.gitDiff(scope, diff.target, diff.path, diff.staged)
        if (result.diff === '') {
          ...
          const other = await api.gitDiff(scope, diff.target, diff.path, !diff.staged)
        ...
        if (diff.untracked === true && !diff.staged) {
          const inventory = await api.gitInventory(scope)
          const repository = inventory.repositories.find(candidate => candidate.id === diff.target.repositoryId)
          const worktree = repository?.worktrees.find(candidate => candidate.id === diff.target.worktreeId)
          if (worktree === undefined) throw new Error('Git target is no longer available')
          const text = await api.fsRead(scope, resolveSidebarPath(worktree.path, diff.path))
          if (!cancelled) {
