...
/** Expanded catalog owners that are reachable through expanded ancestors. */
function visibleExpandedParents(
  ...
  const visit = (parentId: string): void => {
    if (!expanded.has(parentId) || visible.has(parentId)) return
    visible.add(parentId)
    for (const entry of catalogs[parentId]?.entries ?? []) {
      if (entry.kind === 'child' && entry.hasChildren) visit(entry.id)
    }
  }
  ...
  return (
    <>
      ...
      {visibleEntries.map((entry) => {
        ...
        const branchOpen = !knownLeaf && expanded.has(entry.id)
        ...
        return (
          <div key={entry.id} className={css.subagentNode}>
            <div
              ...
              data-subagent-id={entry.id}
              {...knownLeaf ? {} : { 'aria-expanded': branchOpen }}
            ...
            >
              {!knownLeaf && (
                <button
                  type="button"
                  className={clsx(css.subagentDisclosure, branchOpen && css.subagentDisclosureOpen)}
                  aria-label={t(branchOpen ? 'subagentCollapseBranch' : 'subagentExpandBranch', { name: label })}
                  onClick={(event) => {
                    event.stopPropagation()
                    onToggle(entry.id)
                  }}
                >
                  <IconChevronRightOutline14 size={12} aria-hidden />
                </button>
              )}
              <StateDot
...
export function SubagentView(props: {
  ...
  const toggleBranch = useCallback((parentSessionId: string): void => {
    setExpanded((current) => {
  ...
  const rootOpen = rootId !== undefined && expanded.has(rootId)
  const live = useSubagentLive(rootId, active && rootOpen)
  ...
  // Only visible expanded branches consume live membership. Collapsing an
  // ancestor releases every hidden descendant while preserving its local
  // disclosure state for a later re-open.
  const visibleExpanded = useMemo(
    () => visibleExpandedParents(catalogs, rootId, expanded),
    [catalogs, rootId, expanded],
  )
  useEffect(() => {
    const desired = active ? visibleExpanded : new Set<string>()
    for (const id of [...observedRef.current]) {
      if (!desired.has(id)) observe(id, false)
    }
    for (const id of desired) {
      if (!observedRef.current.has(id)) observe(id, true)
    }
  }, [visibleExpanded, active, observe])
  ...
  const readyEmpty = rootCatalog?.state === 'ready'
    ...
    && directChildren(byId, rootId ?? '').length === 0
  const rootExpandable = summaryBackedLoading
    || (rootCatalog?.entries.some(entry => entry.kind === 'child') ?? false)
    || directChildren(byId, rootId ?? '').length > 0
  ...
  const onTreeKeyDown = useCallback((event: KeyboardEvent<HTMLDivElement>): void => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      if (id !== undefined
        && ((event.key === 'ArrowRight' && open === 'false')
          || (event.key === 'ArrowLeft' && open === 'true'))) {
        event.preventDefault()
        toggleBranch(id)
      }
    }
  }, [focusAt, toggleBranch])
        ...
        >
          {rootId !== undefined && rootSummary !== undefined && (
            <div
              ...
              data-subagent-id={rootId}
              aria-expanded={rootExpandable ? rootOpen : undefined}
            ...
            >
              {rootExpandable && (
                <button
                  type="button"
                  className={clsx(css.subagentDisclosure, rootOpen && css.subagentDisclosureOpen)}
                  aria-label={t(rootOpen ? 'subagentCollapseBranch' : 'subagentExpandBranch', {
                    name: rootSummary.displayTitle !== '' ? rootSummary.displayTitle : t('subagentMainAgent'),
                  })}
                  onClick={(event) => {
                    event.stopPropagation()
                    toggleBranch(rootId)
