...
/** Opaque repository and worktree selection validated by the host. */
export interface GitTarget {
  repositoryId: string
  worktreeId: string
}

/** One usable checkout in the Git workspace inventory. */
...
/** One root repository or declared submodule in the inventory. */
...
/** Authoritative repositories and worktrees discovered for one session cwd. */
...
export interface GitLogEntry {
  ...
  refs: string
  /** Full parent hashes in Git order. */
  parents: string[]
}

/** Source-control history filters and page position. */
...
/** One history page plus lookahead metadata. */
export interface GitLogPage {
  entries: GitLogEntry[]
  hasMore: boolean
}

/** Checkout branches and grouped history refs. */
...
/** Add one opaque Git target to a scoped request. */
function gitPayload(scope: SessionScope, target: GitTarget, extra: Record<string, unknown>): Record<string, unknown> {
  return scopePayload(scope, { target, ...extra })
}
...
export const api = {
  ...
  gitInventory: (scope: SessionScope, refresh = false, signal?: AbortSignal) =>
    call<GitWorkspaceInventory>('git.inventory', scopePayload(scope, { refresh }), signal),
  gitStatus: (scope: SessionScope, target: GitTarget, signal?: AbortSignal) =>
    call<GitStatusResult>('git.status', gitPayload(scope, target, {}), signal),
  gitDiff: (scope: SessionScope, target: GitTarget, path: string | undefined, staged: boolean, signal?: AbortSignal) =>
    call<{ diff: string }>('git.diff', gitPayload(scope, target, { ...(path !== undefined ? { path } : {}), staged }), signal),
  gitStage: (scope: SessionScope, target: GitTarget, path?: string) =>
    call<{ ok: true }>('git.stage', gitPayload(scope, target, { ...(path !== undefined ? { path } : {}) })),
  gitUnstage: (scope: SessionScope, target: GitTarget, path?: string) =>
    call<{ ok: true }>('git.unstage', gitPayload(scope, target, { ...(path !== undefined ? { path } : {}) })),
  gitStagePaths: (scope: SessionScope, target: GitTarget, paths: readonly string[]) =>
    call<{ ok: true }>('git.stage-paths', gitPayload(scope, target, { paths })),
  gitUnstagePaths: (scope: SessionScope, target: GitTarget, paths: readonly string[]) =>
    call<{ ok: true }>('git.unstage-paths', gitPayload(scope, target, { paths })),
  gitCommit: (scope: SessionScope, target: GitTarget, message: string) =>
    call<{ ok: true }>('git.commit', gitPayload(scope, target, { message })),
  gitBranch: (scope: SessionScope, target: GitTarget, signal?: AbortSignal) =>
    call<GitBranchResult>('git.branch', gitPayload(scope, target, {}), signal),
  gitCheckout: (scope: SessionScope, target: GitTarget, branch: string) =>
    call<{ ok: true }>('git.checkout', gitPayload(scope, target, { branch })),
  /** Filtered commit history with one-row lookahead pagination. */
  gitLog: (scope: SessionScope, target: GitTarget, query: GitLogQuery, signal?: AbortSignal) =>
    call<GitLogPage>('git.log', gitPayload(scope, target, { query }), signal),
  /** Full patch text of one commit (diff display for the history rows). */
  gitCommitDiff: (scope: SessionScope, target: GitTarget, hash: string, signal?: AbortSignal) =>
    call<{ diff: string }>('git.commit-diff', gitPayload(scope, target, { hash }), signal),
  /** Discard the worktree changes of one file (the index is untouched). */
  gitDiscard: (scope: SessionScope, target: GitTarget, path: string) =>
    call<{ ok: true }>('git.discard', gitPayload(scope, target, { path })),
  /** Revert one commit onto the current branch. */
  gitRevert: (scope: SessionScope, target: GitTarget, hash: string) =>
    call<{ ok: true }>('git.revert', gitPayload(scope, target, { hash })),
  /** Cherry-pick one commit onto the current branch. */
  gitCherryPick: (scope: SessionScope, target: GitTarget, hash: string) =>
    call<{ ok: true }>('git.cherry-pick', gitPayload(scope, target, { hash })),
