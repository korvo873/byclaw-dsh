 ...
 * - Any other `tabId` is a UI-tab terminal. Normal tabs connect with
 *   `?tab=...&sessionId=...&cwd=...`; Worktrees navigation tabs replace cwd
 *   with opaque repository/worktree IDs. A close frame schedules a 0-ms
 *   close; a park frame marks the pty as parked; a bare socket drop gets the
 *   host's reconnect grace.
 */
...
import type { TerminalGitTarget } from './terminal-target.ts'
...
/** Build the terminal WebSocket URL without placing a worktree path on the wire. */
export function terminalSocketUrl(
  scope: SessionScope,
  tabId: string,
  target?: TerminalGitTarget,
  origin = location.origin,
): string {
  const url = new URL('/sidebar/ws/terminal', origin)
  ...
  if (isAgentTabId(tabId)) {
    ...
    if (target === undefined) {
      ...
      params.set('repositoryId', target.repositoryId)
...
export function TerminalView(props: { scope: SessionScope; tabId: string; store: SidebarStore; target?: TerminalGitTarget }) {
  const { scope, tabId, store, target } = props
  const targetRepositoryId = target?.repositoryId
  ...
  useEffect(() => {
    ...
    const wsUrl = (): string => {
      ...
      const socketTarget = targetRepositoryId === undefined || targetWorktreeId === undefined
        ? undefined
        : { repositoryId: targetRepositoryId, worktreeId: targetWorktreeId }
      return terminalSocketUrl(scope, tabId, socketTarget)
    }
  ...
  }, [scope.sessionId, scope.cwd, tabId, store, targetRepositoryId, targetWorktreeId])
